...
AmfitrackDriver::VRPose AmfitrackDriver::PoseHelper::GetSourcePose(VRPose &generic_tracker_pose, VRPose &tracker_pose_in_steamvr)
{
    ...
    HmdVector3_t amfitrack_tracker_position = {
        generic_tracker_pose.Position.v[0],
        -generic_tracker_pose.Position.v[1], // Flip Y
...
AmfitrackDriver::VRPose AmfitrackDriver::PoseHelper::CalculateControllerPose(VRPose &steamVR_source_position, VRPose &controller_pose)
{
    ...
    HmdVector3_t amfitrack_controller_position = {
        controller_pose.Position.v[0],
        -controller_pose.Position.v[1], // Flip Y-axis
        -controller_pose.Position.v[2]  // Flip Z-axis
    };

    // Compute the controller's position in SteamVR space
    HmdVector3_t controller_position_in_vr_space = {
        steamVR_source_position.Position.v[0] + amfitrack_controller_position.v[0],
        steamVR_source_position.Position.v[1] + amfitrack_controller_position.v[1],
        steamVR_source_position.Position.v[2] + amfitrack_controller_position.v[2]};
