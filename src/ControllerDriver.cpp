...
ControllerDriver::ControllerDriver(uint8_t DeviceID, ControllerDriver::Handedness handedness) : deviceID_(DeviceID), handedness_(handedness)
{
}
...
EVRInitError ControllerDriver::Activate(uint32_t unObjectId)
{
	...
	PropertyContainerHandle_t props = vr::VRProperties()->TrackedDeviceToPropertyContainer(this->device_index_); // this gets a container object where you store all the information about your driver
	...
	// The following properties are ones I tried out because I saw them in other samples, but I found they were not needed to get the sample working.
...
void ControllerDriver::RunFrame()
{
	...
	bool ButtonPressed = CHECK_BIT(gpio_state, 0);

	if (this->handedness_ == Handedness::RIGHT)
	{
		if (ButtonPressed)
		{
			VRDriverLog()->Log("Right sensor button pressed");
		}
		VRDriverInput()->UpdateBooleanComponent(this->a_button_click_component_, ButtonPressed, 0);
		VRDriverInput()->UpdateBooleanComponent(this->a_button_touch_component_, ButtonPressed, 0);
	}
	else if (this->handedness_ == Handedness::LEFT)
	{
		if (ButtonPressed)
		{
			VRDriverLog()->Log("Left sensor button pressed");
		}
		VRDriverInput()->UpdateBooleanComponent(this->b_button_click_component_, ButtonPressed, 0);
		VRDriverInput()->UpdateBooleanComponent(this->b_button_touch_component_, ButtonPressed, 0);
	}

	// if (GetAsyncKeyState(0x44 /* D acting as Trigger */) != 0)
	// {
	// 	VRDriverInput()->UpdateBooleanComponent(this->trigger_click_component_, 1, 0);
	// 	VRDriverInput()->UpdateBooleanComponent(this->trigger_touch_component_, 1, 0);
	// }
}
