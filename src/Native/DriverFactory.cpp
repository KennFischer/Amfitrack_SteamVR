...
static std::shared_ptr<AmfitrackDriver::IVRDriver> driver;
...
void* HmdDriverFactory(const char* interface_name, int* return_code) {
	if (std::strcmp(interface_name, vr::IServerTrackedDeviceProvider_Version) == 0) {
		if (!driver) {
			...
			driver = std::make_shared<AmfitrackDriver::VRDriver>();
		}
...
std::shared_ptr<AmfitrackDriver::IVRDriver> AmfitrackDriver::GetDriver() {
	return driver;
}
